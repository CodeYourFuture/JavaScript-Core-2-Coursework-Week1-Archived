...
*/
const age = 35
let thirtyFiveOrOlder = friends.filter(element => element.age >= age)
...
let friendName = friends.filter(element => element.colleagues.some(element => element.name === 'Stacie Villarreal'))
friendsWhoAreColleaguesOfStacie = friendName.map(element => `${
    element.name.first
} ${
    element.name.last
}`)
...
let multitaskingColleague;

multitaskingColleague = friends.forEach(element =>{
  element.colleagues.filter(element =>{
      if(element.skills.some(element => element === 'Multi-tasking'))
