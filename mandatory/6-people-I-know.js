...
//   if (element.age >= 35) {
//     thirtyFiveOrOlder.push(element.name.first);
//   }
// }
...
let powerNetEmails = [];
let emailAddresses = friends
  .filter((element) => element.company === "POWERNET")
  .map((element) => {
    return element.email;
  });
powerNetEmails.push(...emailAddresses);
...
for (el of friends) {
  for (element of el.colleagues) {
    if (element.name === "Stacie Villarreal")
...
for (el of friends) {
  for (coll of el.colleagues) {
    if (coll.skills.includes("Multi-tasking")) {
      colleaguesWhoCanMultitask.push(`${coll.name}`);
    }
  }
}
/* ======= TESTS - DO NOT MODIFY ===== 
