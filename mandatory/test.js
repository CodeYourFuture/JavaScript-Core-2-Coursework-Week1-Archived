...
//logs correct names separately, no quotes
// console.log(friends
//   .filter((friend) => friend.age > 35)
//    .map((friend) => friend.name.first));

// friends.filter((friend) => friend.colleagues.includes("Stacie")).map(friend =>console.log(friend.name.first));

console.log(friends
  .filter((friend) => friend.colleagues.some((colleague) => colleague.name === "Stacie"))
  .map(friends.colleague));
