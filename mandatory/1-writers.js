...
writers.forEach(item => {
  if (item.age < 50 && item.age >= 40 && !item.alive) console.log(`Writer ${item.firstName} ${item.lastName} died at ${item.age} years old.`);
});
/*
Exercise 3:
...
*/
writers.forEach(item => {
  if(item.age < 50 && item.age >= 40 && item.alive) console.log(`Hi, my name is ${item.firstName} ${item.lastName}. I am ${item.age} years old.`);
})
