...
function logDeadWritersInTheirForties() {
   writers
    .filter((keys) => keys.age >= 40 && keys.age <= 49 && keys.alive === false)
    .map(
      (keys) =>
        console.log( `Writer ${keys.firstName} ${keys.lastName} died at ${keys.age} years old.`)
    );
}
...
function logAliveWritersInTheirForties() {
  writers
    ...
    .map(
      (keys) =>
        console.log( `Hi, my name is ${keys.firstName} ${keys.lastName}. I am ${keys.age} years old.`)
    );
}
