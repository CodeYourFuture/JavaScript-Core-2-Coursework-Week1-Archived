...
function eligibleStudents(attendances) {                          //declares the function with a parameter 
    return attendances
    .filter((student) =>student.attendance >=8) //code bloc of function requires code to RETURN the parameter attendances of each student via .filter.
    .map((student)=>student.name);                                                              // and only if attendance is over 8.
                                  // .map is a method that returns a new array containing the newly filtered students. 
}
