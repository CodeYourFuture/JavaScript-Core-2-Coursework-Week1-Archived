...
function chooseMeal(mealArray) {
  // if meal array is empty return Nothing :(
  if (mealArray.length === 0) {
    return "Nothing :(";
    // if meal array is only one meal return that meal
  } else if (mealArray.length === 1) {
    return mealArray[0].name;
    // if meal array is bigger than 1 return the second cheapest item
  } else {
    // sort meals into ascending order to take second lowest
    let ascendingMealPrices = mealArray.sort((a, b) =>
      a.price > b.price ? 1 : b.price > a.price ? -1 : 0
    );
    // return the second lowest meal
    return ascendingMealPrices[1].name;
  }
}
