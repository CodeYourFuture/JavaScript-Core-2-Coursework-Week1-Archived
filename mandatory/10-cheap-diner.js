...
function chooseMeal(mealArray) {
  ///First we sort meals according to their price from low/cheap to high/expensive
  mealArray.sort((meal1, meal2) => meal1.price - meal2.price);
  ///////this will return "Nothing" if array is empty
  if (mealArray.length == 0) {
    return "Nothing :(";
    ///if array has one meal name this it will return it's name which will be at index=0
  } else if (mealArray.length == 1) {
    return mealArray[0].name;
    ///if array has more than one meal. It will return the second cheapest meal in the array which will always be at index=1
  } else {
