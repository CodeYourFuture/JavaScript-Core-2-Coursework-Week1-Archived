...
function chooseMeal(mealArray) {
  ...
  if (mealArray.length == 0) {
    mealToChoose = "Nothing :(";
    console.log(mealToChoose);
  } else if (mealArray.length > 1) {
    let comparisonArr = [];
    mealArray.forEach((meal) => comparisonArr.push(meal["price"]));
    let min = Math.min.apply(Math, comparisonArr);
    console.log(min);

    let index = comparisonArr.indexOf(min);

    comparisonArr.splice(index, 1);
    console.log(`comparison Arr: ${comparisonArr}`);

    let minToo = Math.min.apply(Math, comparisonArr);
    console.log(`min Too: ${minToo}`);

    let filtered = mealArray.filter((meal) => meal["price"] == minToo);
    console.log(filtered);
    filtered.forEach((meal) => (mealToChoose = meal["name"]));
    console.log(mealToChoose);
  } else {
    mealArray.forEach((meal) => (mealToChoose = meal["name"]));
    console.log(mealToChoose);
  }
