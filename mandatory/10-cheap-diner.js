...
function chooseMeal(mealArray) {
  if (mealArray.length === 0) {
    ...
    let mostCheapItem = mealArray[0];
    let secondMostCheapItem = mealArray[1];
    mealArray.forEach((menuItem) => {
      if (menuItem.price < mostCheapItem.price) {
        secondMostCheapItem = mostCheapItem;
        mostCheapItem = menuItem;
      } else if (
        menuItem.price > mostCheapItem.price &&
        menuItem.price < secondMostCheapItem.price
      ) {
        secondMostCheapItem = menuItem;
      }
    });
    return secondMostCheapItem.name;
  }
}
