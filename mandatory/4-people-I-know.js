...
let powerNetEmails = people.filter((friend) => friend.company === "POWERNET").map((email2) => email2.email).sort();
...
let friendsWithStacie = people.filter((friend) => friend.friends.find((friend) => friend.name === "Stacie Villarreal")
...
let friendsWhoCanMultitask = people.flatMap((friend) => friend.friends.filter((friend) => friend.skills.includes("Multi-tasking"))
