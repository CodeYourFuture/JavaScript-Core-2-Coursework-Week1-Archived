...
let powerNetEmails = people.filter(x => x.company === 'POWERNET').map(x => x.email).reverse();
...
let friendsWhoCanMultitask = people.map(x => x.friends.filter(a => a.skills.includes('Multi-tasking'))).flat().map(x => x.name);
