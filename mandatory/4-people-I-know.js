...
let thirtyFiveOrOlder = people.filter(x => x.age >= 35);
...
let powerNetEmails=[];
let testIt=people.filter(x => x.company === "POWERNET");
powerNetEmails=testIt.map(x => x.email).sort();
...
let checkForStacy=people.filter(x => x.friends.find( x=> x.name === "Stacie Villarreal"));
friendsWithStacie=checkForStacy.map(function (x){
...
let checkForMulti=people.map(x => x.friends.filter( y=> y.skills.includes("Multi-tasking"))).flat();
friendsWhoCanMultitask=checkForMulti.map(x=>x["name"]);
