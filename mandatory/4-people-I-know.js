...
thirtyFiveOrOlder = people.filter(person => person.age > 35);
/*
...
let powerNetEmails = [];
powerNetEmails = people.filter(per => {
    if (per.company === "POWERNET") {
        return per.email;
    }
}).map(per => per.email).sort();
...
friendsWithStacie = people.filter(s => {
    if (s.name.first === 'Clay' && s.name.last === 'Livingston') {
        return s.friends;
    }
}).filter(s => s.name);
