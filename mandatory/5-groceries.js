...
for (const day in weeklyMealPlan) {
  const eachDayArray = weeklyMealPlan[day];
  eachDayArray.map((ingredient) => {
...
for (const day in weeklyMealPlan) {
  if (day === "saturday" || day === "sunday") {
    const weekendFilteredArray = weeklyMealPlan[day];
    weekendFilteredArray.map((element) => {
...
for (const day in weeklyMealPlan) {
  const dailyIngredients = weeklyMealPlan[day];
  const numberOfIngredientsPerDay = dailyIngredients.length;
  numberOfItemsPerWeek[day] = numberOfIngredientsPerDay;
}
