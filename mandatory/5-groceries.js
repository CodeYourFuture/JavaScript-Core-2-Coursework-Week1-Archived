...
let allingredients = Object.values(weeklyMealPlan);
for (ingredient of allingredients) {
  for (let i = 0; i < ingredient.length; i++) {
    if (!weeklyGroceriesToBuy.includes(ingredient[i])) {
      weeklyGroceriesToBuy.push(ingredient[i]);
    }
  }
}
/*
...
for (element in weeklyMealPlan) {
  if (
    (element === "saturday" || element === "sunday") &&
    weeklyMealPlan[element].length > 0
  ) {
