...
for (let day in weeklyMealPlan) {
  for (let i = 0; i < weeklyMealPlan[day].length - 1; i++) {
    s.push(weeklyMealPlan[day][i]);
  }
}
new Set(s);
weeklyGroceriesToBuy = Array.from(s);
...
for (let day in weeklyMealPlan) {
  if (day.toLowerCase() == "saturday" || day.toLowerCase() == "sunday") {
    for (let i = 0; i < weeklyMealPlan[day].length - 1; i++) {
      w.push(weeklyMealPlan[day][i]);
    }
  }
}
new Set(w);
weekendGroceriesToBuy = Array.from(w);
