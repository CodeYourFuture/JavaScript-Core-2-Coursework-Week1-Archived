...
for (const dayOfTheWeek in weeklyMealPlan) {
  // iterates through weekly meal plan object
  const ingredientsOfTheDay = weeklyMealPlan[dayOfTheWeek]; // assigns items to a variable
  weeklyGroceriesToBuy = weeklyGroceriesToBuy.concat(ingredientsOfTheDay); // puts items in array
}

weeklyGroceriesToBuy = [...new Set(weeklyGroceriesToBuy)]; // removes duplicates and adds to array
console.log("Weekly Groceries:", weeklyGroceriesToBuy);
...
for (const dayOfTheWeek in weeklyMealPlan) {
  if (dayOfTheWeek === "saturday" || dayOfTheWeek === "sunday") {
    const ingredientsOfTheDay = weeklyMealPlan[dayOfTheWeek]; // assigns items to a variable
    console.log(ingredientsOfTheDay);
    weekendGroceriesToBuy = weekendGroceriesToBuy.concat(ingredientsOfTheDay); // puts items in array
  }
}

weekendGroceriesToBuy = [...new Set(weekendGroceriesToBuy)]; // removes duplicates and adds to array
console.log("Weekend Groceries", weekendGroceriesToBuy);
